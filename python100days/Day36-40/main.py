...
select conid as is, conname as name, contel as tel. conemail as email 
...
def delete_contacter(con, contacter):
    try:
        with con.cursor() as cursor:
            if cursor.execute(DELETE_CONTACTER,
                              (contacter.id, )) == 1:
                print('联系人已经删除！')
    except pymysql.MySQLError as err:
        print(err)
